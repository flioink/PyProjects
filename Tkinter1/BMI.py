...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.height_int = ctk.IntVar(value=179)
        self.weight_float = ctk.DoubleVar(value=82)
...
class WeightInput(ctk.CTkFrame):
    ...
    def update_weight(self, info=None):
        ...
        if info:
            ...
            if self.metric_bool.get():
                self.output_string.set(f"{round(self.weight_float.get(), 3)}kg")
...
class HeightInput(ctk.CTkFrame):
    def __init__(self, parent, height_int, metric_bool):
        ...
        output_text = ctk.CTkLabel(
            self,
            textvariable=self.output_sting,
            text_color=BLACK,
            font=ctk.CTkFont(family=FONT, size=INPUT_FONT_SIZE))
    ...
    def update_text(self, amount):
        if self.metric_bool.get():
            ...
            self.output_sting.set(f"{int(feet)}' {int(inches)}''")
